...
- **The extension now starts with VS Code** (`onStartupFinished`). Without it, activation waited for one of its views to be opened, so the first-run prompt could only reach someone who had already found the extension.
- **The setup confirmation is a modal.** It was a quick pick's one-line detail, truncated after "Source:" — hiding that detection can propose an enterprise repo, a design-themes repo and an addons folder from anywhere on the machine, all written at user scope on one keystroke.
- **Setup's custom addons folder now reaches the active version**, not just the user-level default. Repository discovery reads the version's own copy, so setup reported success while Create Project searched a `./custom-addons` that never existed.
- The version multi-select offers versions from your addon repositories on a first run, where there are no projects to read them from yet — and **`master` is never the pre-ticked default**, so the out-of-the-box choice no longer spends ~2 GB on the development branch.
- The one-copy-per-branch dialog no longer promises directories and lists none when nothing is mapped yet, names the original checkout by absolute path, and says *copies* on its button instead of *worktrees*.
- The menu entry that removes the copies now reads **Use a Single Checkout** instead of **Use One Copy Per Branch**.
- **A legacy version running out of the source repository is diagnosed as such even when its interpreter is also missing** — the exact 1.2 shape, previously reported as merely `unprovisioned` and stripped of the wording explaining why it is unsafe.
- Upgrade branch pickers preselect their seeded branch (a description alone never moved the selection), and the *upgrading to* list no longer offers the branch just chosen as *upgrading from*.
- Setup no longer asks *provision or profile only?* about a version you already chose to build.
- Three more places that said "No projects found. Create a project first." now offer **Create Project** instead of stacking a dead-end toast on the one that does.
...
- Freeing a branch from the source checkout is explicit: moving it to another branch is offered first, detaching second — with both consequences stated (the source cannot return to that branch until the worktree is removed, and commits made detached belong to no branch) — and a dirty checkout is refused with its changed files named. Nothing is detached, moved or stashed silently.
- The repo views, Modules view, module scaffolding and generated workspace follow the active version's copies, and opening a file from another version's copy offers to reopen the active one.
...
- **Changing a database's version left its branch label stale.** `Change Version` set `versionId` but never updated `branchName`, and the Databases view shows `branchName` whenever it differs from the version's branch — so a database created against 17.0 and moved to v19 rendered as `17.0 • Odoo 19.0`.
- **The switch notification claimed a branch checkout that was not going to happen.** A provisioned version's worktree is already on the right branch, and a version whose directories are missing cannot be checked out at all; both were reported as `branch "19.0"`. The summary now distinguishes the three cases: `its existing "19.0" worktree`, `core branch "19.0"`, and `core repositories for "19.0" are missing`, the last of which is now reported as a failure instead of silently doing nothing.
- **Re-provisioning a version whose folder had been deleted failed with *"'odt/19.0' is already used by worktree at …"***. git keeps a branch reserved for a worktree whose directory no longer exists — the record is only marked prunable — so the version blocked its own rebuild. Provisioning now prunes stale worktree records first. A branch still held by a *live* worktree elsewhere (typically one built under an older provisioning root) is adopted rather than duplicated, and the reuse is reported.
